...
- **External-boundary `Interface` + `InMemory*` + production-impl pattern.** Server services that wrap an external boundary (DB, HTTP API, WASM crypto, Discord client) export the boundary as a TypeScript interface and ship two implementations: an `InMemory*` for unit/route tests and a production impl (`Prisma*`, an HTTP wrapper, `Cardano*` for WASM, etc.). `apps/server/src/index.ts` (`buildServer`) accepts the abstract interface in `BuildServerOptions` so route tests inject the in-memory variant; the prod entrypoint wires the heavy implementation behind an env-gated `getPrisma()` / WASM constructor. This makes the entire fetch/cache/verify path testable without spinning up Postgres, hitting the network, or paying WASM cold-start per assertion. Established by `UserStore` ↔ `PrismaUserStore` / `InMemoryUserStore`, `NonceStore` ↔ `InMemoryNonceStore`, `BattleStore` ↔ `PrismaBattleStore` / `InMemoryBattleStore`, `WalletAuthVerifier` ↔ `CardanoWalletAuthVerifier` (route tests use a script-table stub; a separate suite exercises the real WASM impl), `NftSnapshotStore` ↔ `PrismaNftSnapshotStore` / `InMemoryNftSnapshotStore`, and `BlockfrostClient` ↔ `BlockfrostHttpClient` (tests use a `StubBlockfrostClient`). Default to this split for any new external dependency.
