...
- **STORY-01** — Monorepo scaffold + green CI
  ...
  - :white_check_mark: **TASK-005** — Scaffold apps/discord stub + apps/mobile placeholder  `med` `small` _(apps/discord, apps/mobile)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/42)  
    _depends on: TASK-002_
  ...
  - :white_check_mark: **TASK-006** — ESLint + Prettier root config (flat config)  `med` `small` _(root)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/43)  
...
- **STORY-06** — Phaser battle scene
  ...
  - :white_check_mark: **TASK-019** — Web: turn animation placeholders (sprite shake + flash)  `med` `small` _(apps/web)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/47)  
...
- **STORY-07** — First PvE opponent + win/loss screens
  ...
  - :white_check_mark: **TASK-021** — Web: win/loss screens + battle history list  `med` `small` _(apps/web)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/48)  
...
- **STORY-09** — NFT discovery service (Blockfrost) with allow-list + caching
  ...
  - :white_check_mark: **TASK-026** — Server: GET /api/roster (free starter + NFT crews)  `high` `medium` _(apps/server)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/30)  
...
- **STORY-10** — Trait → stat derivation pipeline + admin tooling
  - :white_check_mark: **TASK-027** — Trait→stat derivation: deterministic mapper + collection registry  `high` `large` _(apps/server, packages/db, packages/shared)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/38)  
  ...
  - :white_check_mark: **TASK-028** — Admin: CLI tool to register a new collection (policyId + ruleJson)  `med` `medium` _(apps/server, packages/db)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/49)  
...
- **STORY-11** — XP, level-up, attribute training UI
  - :white_check_mark: **TASK-029** — Server: XP grant on battle end + level-up curve  `high` `medium` _(apps/server, packages/core)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/39)  
  ...
  - :white_check_mark: **TASK-030** — Web: crew detail screen + attribute training UI  `med` `medium` _(apps/web)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/50)  
...
- **STORY-12** — Item system: drops, inventory, apply-to-crew
  - :white_check_mark: **TASK-031** — Server: item drops on battle win + inventory endpoint  `med` `medium` _(apps/server, packages/content)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/51)  
    _depends on: TASK-029_
  ...
  - :white_check_mark: **TASK-032** — Web: inventory UI + apply flow  `med` `small` _(apps/web)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/52)  
...
- **STORY-13** — Battle stats + replay viewer
  - :white_check_mark: **TASK-033** — Server: aggregate per-crew W/L, K/D + GET /api/stats  `med` `medium` _(apps/server)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/53)  
    _depends on: TASK-029_
  ...
  - :white_check_mark: **TASK-034** — Web: replay viewer (re-render BattleEvents through engine)  `med` `large` _(apps/web, packages/core)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/59)  
...
- **STORY-14** — Discord bot scaffold + Discord↔wallet identity linking
  - :white_check_mark: **TASK-035** — Discord bot scaffold + slash command registration tooling  `high` `medium` _(apps/discord)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/44)  
    _depends on: TASK-005_
    ...
    > not on bot startup.
  - :white_check_mark: **TASK-036** — Server: /api/discord/link-token + /api/discord/link-claim  `high` `medium` _(apps/server)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/40)  
    _depends on: TASK-023_
  ...
  - :white_check_mark: **TASK-037** — Discord bot: /link command + DM flow → server claim  `high` `medium` _(apps/discord)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/45)  
...
- **STORY-15** — Slash commands suite: /team /battle /move /switch /forfeit /stats
  - :white_check_mark: **TASK-038** — Discord: /team + /battle + /stats slash commands  `high` `medium` _(apps/discord, apps/server)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/54)  
  ...
  - :white_check_mark: **TASK-039** — Discord: /move + /switch + /forfeit (turn actions)  `high` `medium` _(apps/discord, apps/server)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/55)  
...
- **STORY-16** — Embed-based battle renderer + async match state machine
  - :white_check_mark: **TASK-040** — Embed renderer (HP bars, move announcement, type-effectiveness)  `high` `medium` _(apps/discord)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/56)  
    _depends on: TASK-038_
  ...
  - :white_check_mark: **TASK-041** — Async match state machine (Battle messageId, edit lifecycle)  `high` `large` _(apps/server, apps/discord)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/58)  
...
Wrap the web build for Android via Capacitor (iOS deferred — no
macOS access in the autonomous pipeline); sort out mobile wallet
UX (CIP-45 / WalletConnect). Add async PvP — challenge a friend
...
- **STORY-17** — Capacitor wrap (Android) + mobile wallet UX
  - :black_circle: **TASK-042** — apps/mobile: Capacitor Android scaffold pointing at apps/web build  `high` `medium` _(apps/mobile, apps/web)_  
    _depends on: TASK-021_
    > apps/mobile: install @capacitor/core + @capacitor/cli +
    > @capacitor/android. Add capacitor.config.ts with
    > webDir = "../web/dist" and appId/appName set. Run
    > `npx cap add android` (results committed). Add scripts:
    > build (npm run build -w apps/web && npx cap sync),
    > open:android. Add the Android generated-artefact paths
    > from CLAUDE.md → Scaffolding hygiene to .gitignore in the
    > same PR (android/build/, android/.gradle/,
    > android/app/build/). iOS is intentionally out of scope —
  ...
  - :black_circle: **TASK-043** — Mobile wallet UX: CIP-45/WalletConnect bridge + fallback (Android)  `high` `large` _(apps/mobile, apps/web)_  
    ...
    > WalletConnect session. Use @walletconnect/universal-provider
    > with the Cardano namespace adapter. For Android wallets
    > without CIP-45 support, deep-link to a known dApp browser
    > (e.g. Eternl mobile) via Android intent / custom scheme;
    > document fallback in docs/MOBILE.md. iOS path is out of
    > scope (see EPIC-06 note).

- **STORY-18** — Async PvP: challenge-by-link + match queue + persistence
  - :white_check_mark: **TASK-044** — Server: PvP challenge create/accept + match queue  `high` `large` _(apps/server)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/41)  
  ...
  - :white_check_mark: **TASK-045** — Web: PvP UI (generate challenge link, accept, in-progress list)  `high` `medium` _(apps/web)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/46)  
...
- **STORY-19** — Leaderboard + season/ladder skeleton
  - :white_check_mark: **TASK-046** — Server: ELO calc + Season + per-season leaderboard endpoint  `med` `medium` _(apps/server, packages/db)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/60)  
    _depends on: TASK-044_
  ...
  - :white_check_mark: **TASK-047** — Web + Discord: leaderboard views  `med` `small` _(apps/web, apps/discord)_ · [PR](https://github.com/g-chappell/pirate-battle/pull/61)  
