...
- **STORY-01** — Monorepo scaffold + green CI
  ...
  - :black_circle: **TASK-003** — Scaffold apps/server (Fastify) with /health endpoint  `high` `small` _(apps/server)_  
...
- **STORY-02** — Postgres schema + Prisma migrations
  ...
  - :black_circle: **TASK-008** — Prisma schema: battles + battle_events + initial migration  `high` `medium` _(packages/db)_  
...
- **STORY-07** — First PvE opponent + win/loss screens
  - :black_circle: **TASK-020** — Server: AI opponent + battle create/resolve endpoints  `high` `large` _(apps/server, packages/core)_  
